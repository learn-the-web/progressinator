...
import phonenumbers
from progress.common.util import classify, NumberFormatter
...
def pretty_percent(value):
    ...
    if value: return NumberFormatter.percent_humanize(value)

@register.filter
def pretty_percent_strip(value):
    """Take a mathematical, database, percent and make it human readable also stripping trailing 0"""
    if value: return NumberFormatter.percent_humanize(value, strip=True)

(function () {
  ...
  const setStudentSection = function (section) {
    if (studentSection) studentSection.querySelector('[aria-pressed="true"]').setAttribute('aria-pressed', false);

    if (!section || section == 'all') {
      studentIds = allStudentsIds;
      if (studentSection) studentSection.querySelector('[data-section="all"]').setAttribute('aria-pressed', true);
    } else {
      studentIds = allStudentsIds.filter((x) => { return (x.section == section || x.section == '---') });
      if (studentSection) studentSection.querySelector(`[data-section="${section}"]`).setAttribute('aria-pressed', true);
    }

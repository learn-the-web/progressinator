...
def index(request):
    ...
    if current_term:
        try:
            user_profile = UserProfile.objects.get(
                user=request.user, current_course__term=current_term
            )
        except:
            user_profile = None
    else:
    ...
    if user_profile:
        response.set_cookie(
            "ltw-course-section",
            f"{user_profile.current_course.slug}-{user_profile.current_section}",
...
def save(request):
    ...
    previous_user_profile = UserProfile.objects.filter(
        user=request.user, current_course__term=current_term
    )
    ...
    user_profile = UserProfile(
        user=request.user,
        current_course_id=course_bits[0].strip(),
        current_section=course_bits[1].strip(),
    )

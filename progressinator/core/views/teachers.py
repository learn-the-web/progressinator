...
from progressinator.common.util import (
    build_dict_index,
    build_queryset_index,
    model_to_dict,
    NumberFormatter,
)
...
def comment_is_different(data1, data2):
    ...
    if data1 and "comment" in data1:
        comment1 = data1["comment"]
    if "details" in data2 and "comment" in data2["details"]:
        comment2 = data2["details"]["comment"]
...
def crud_grades(request, user_grades, assessment_uri_index=None, user_id_index=None):
    ...
    for (i, prog_id) in enumerate(post_user_progress_id):
        ...
        user_progress_model = {
            "grade": decimal.Decimal(the_grade) if the_grade != "" else "",
        ...
        if post_grade[i].strip() != "":
            if prog_id.strip() == "":
                ...
                if assessment_uri_index:
                    current_user_progress = user_grades[
                        assessment_uri_index[user_progress_model["assessment_uri"]]
                    ]
                else:
                    current_user_progress = user_grades[
                        user_id_index[user_progress_model["user_id"]]
                    ]

                if (
                    decimal.Decimal(current_user_progress.grade).quantize(
                        grade_helper.TWO_DECIMALS
                    )
                    != decimal.Decimal(user_progress_model["grade"]).quantize(
                        grade_helper.TWO_DECIMALS
                    )
                    or comment_is_different(
                        current_user_progress.details, user_progress_model
                    )
                    or empty_is_none(current_user_progress.excuse_lateness)
                    != empty_is_none(user_progress_model["excuse_lateness"])
                ):
                    current_user_progress.grade = user_progress_model["grade"]
                    current_user_progress.submitted_by = (
                        f"{request.user.first_name} {request.user.last_name}"
                    )
                    current_user_progress.cheated = False
                    current_user_progress.excuse_lateness = user_progress_model[
                        "excuse_lateness"
                    ]
                    if not isinstance(current_user_progress.details, dict):
                        current_user_progress.details = {}
                    current_user_progress.details["comment"] = user_progress_model[
                        "details"
                    ]["comment"]
            ...
            if prog_id.strip():
                if assessment_uri_index:
                    if user_progress_model["assessment_uri"] in assessment_uri_index:
                        posted_grades["delete"].append(
                            user_grades[
                                assessment_uri_index[
                                    user_progress_model["assessment_uri"]
                                ]
                            ]
                        )
                else:
                    if user_progress_model["user_id"] in user_id_index:
                        posted_grades["delete"].append(
                            user_grades[user_id_index[user_progress_model["user_id"]]]
                        )
...
def courses(request):
    ...
    for term in terms:
        if pendulum.now() >= pendulum.instance(
            datetime.datetime.combine(term.start_date, datetime.time.min)
        ) and pendulum.now() <= pendulum.instance(
            datetime.datetime.combine(term.end_date, datetime.time.max)
        ):
            current_term = term
    ...
    for course in context["courses"]:
        course.data["totals"]["students"] = Course.objects.get(
            pk=course.pk
        ).profiles.count()
...
def course_status(request, term_id, course_id):
    ...
    students = (
        UserProfile.objects.filter(current_course=course)
        .select_related("user")
    ...
    for student in students:
        student_grades = (
            grade_helper.calc_grade(g, assessment_index, course.data["assessments"])
            for g in all_grades
        ...
        if student.current_section in max_assessments_per_section:
            student.current_grade_max = max_assessments_per_section[
                student.current_section
            ]
        else:
            student.current_grade_max = 1
        student.current_grade_average = (
            student.current_grade / student.current_grade_max
            if student.current_grade_max > 0
            else 0
        )
        ...
        stats_grade_status[
            grade_helper.grade_as_status(student.current_grade_average)
        ] += 1
    ...
    for prog in all_grades:
        if prog.assessment_uri in assessment_index and prog.grade > 0:
            if (
                "total_students_pass"
                not in course.data["assessments"][assessment_index[prog.assessment_uri]]
            ):
    ...
    for assessment in course.data["assessments"]:
        if "total_students_pass" in assessment:
            assessment["total_students_pass_rate"] = (
                assessment["total_students_pass"] / students.count()
            )
        else:
    ...
    context = {
        ...
        "stats_assessments_no_zeros": len(course.data["assessments"])
        - len(
            [
                a
...
def download(request, term_id, course_id, student_grade_group="all"):
    ...
    response[
        "Content-Disposition"
    ] = f'attachment; filename="{course.slug}-{term.slug}-{student_grade_group}.csv"'
    ...
    for student in students:
        ...
        if (
            student_grade_group == "failures"
            and student.current_grade_average >= decimal.Decimal(0.5)
        ):
            continue
        student_assessment_index = {
            g.assessment_uri: g.grade
            for g in all_grades
            if g.user_id == student.user_id
        }
        student_details = [
            ...
            student.current_section,
            grade_helper.grade_as_letter(student.current_grade_average),
            NumberFormatter.percent_humanize(student.current_grade, small=True),
            grade_helper.grade_as_status(student.current_grade_average),
        ]
        student_grade_data = []
        for assessment in course.data["assessments"]:
            if assessment["assessment_each_algonquin"] > 0:
                if assessment["uri"] in student_assessment_index:
                    student_grade_data.append(
                        NumberFormatter.percent_humanize(
                            student_assessment_index[assessment["uri"]], small=True
                        )
                    )
                else:
                    student_grade_data.append("")
...
def user_grades(request, term_id, course_id, user_id):
    ...
    all_student_courses = (
        UserProfile.objects.filter(user=user_id)
        .exclude(current_course=course)
    ...
    for a in course.data["assessments"]:
        if (
            ...
            and student_profile.current_section in a["due_dates_algonquin"]
        ):
            a["user_due_date_algonquin"] = pendulum.parse(
                a["due_dates_algonquin"][student_profile.current_section],
                tz="America/Toronto",
            )

    if (
        student_profile
        and student_profile.current_section
        and student_profile.current_section in course.data["sections"]
    ):
...
def user_grades_save(request, term_id, course_id, user_id):
    ...
    crud_grades(
        ...
        assessment_uri_index=assessment_uri_index,
    )
...
def assessment_grades(request, term_id, course_id, assessment_id):
    ...
    student_grades = list(
        UserProgress.objects.filter(
            user__in=[s.user.id for s in students], assessment_uri=assessment_id
        )
    )
    assessment_index = build_dict_index(course.data["assessments"], "uri")
    ...
    for student in students:
        ...
        if student.user.id in student_grade_index:
            grade_info["grade"] = model_to_dict(
                student_grades[student_grade_index[student.user.id]]
            )
            ...
            if (
                grade_info["grade"]["details"]
                and "number_of_commits" in grade_info["grade"]["details"]
            ):
                show_markbot_stats = True
                if (
                    grade_info["grade"]["details"]["number_of_commits"] is not None
                    and grade_info["grade"]["details"]["number_of_commits"] is not False
                    and float(grade_info["grade"]["details"]["number_of_commits"]) > 0
                ):
                    markbot_commits_total += grade_info["grade"]["details"][
                        "number_of_commits"
                    ]
                    if (
                        grade_info["grade"]["details"]["number_of_commits"]
                        <= markbot_commits_min
                    ):
                        markbot_commits_min = grade_info["grade"]["details"][
                            "number_of_commits"
                        ]
                    if (
                        grade_info["grade"]["details"]["number_of_commits"]
                        ...
                        markbot_commits_max = grade_info["grade"]["details"][
                            "number_of_commits"
                        ]

            if (
                grade_info["grade"]["details"]
                and "estimated_time" in grade_info["grade"]["details"]
            ):
                show_markbot_stats = True
                if (
                    grade_info["grade"]["details"]["estimated_time"] is not None
                    and grade_info["grade"]["details"]["estimated_time"] is not False
                    and float(grade_info["grade"]["details"]["estimated_time"]) > 0
                ):
                    estimated_time = (
                        float(grade_info["grade"]["details"]["estimated_time"])
                        if float(grade_info["grade"]["details"]["estimated_time"]) <= 50
                        else 50
                    )
                    markbot_time_total += estimated_time
                    if estimated_time <= markbot_time_min:
        ...
        if student.current_section and "due_dates_algonquin" in assessment:
            ...
            if "grade" in grade_info:
                ...
                if (
                    "created" in grade_info["grade"]
                    and grade_info["grade"]["created"]
                    and grade_info["grade"]["created"]
                    > grade_info["user_due_date_algonquin"]
                    and (
                        grade_info["grade"]["excuse_lateness"] == "LATENESS_NOT_EXCUSED"
                        or not grade_info["grade"]["excuse_lateness"]
                    )
                ):
...
def assessment_grades_save(request, term_id, course_id, assessment_id):
    if request.method != "POST" and "user_progress_id" not in request.POST:
        return redirect(
            "core:teacher_assessment_grades",
            term_id=term_id,
            course_id=course_id,
